...
// Scroll-reveal wrapper — the section starts faded/lowered and settles into
// place the first time it crosses into the viewport. Tasteful (480ms,
// translateY(16px) at most), fires once per section, and costs nothing on
// sections already in view on load (they reveal immediately).
function Reveal({ children, as: Tag = 'div', className = '', ...rest }) {
  ...
  return (
    <Tag ref={ref} className={`${className} ${visible ? 'reveal-in' : 'reveal-init'}`} {...rest}>
      {children}
    </Tag>
  );
}

// Rotating one-line ticker for the hero's marketing column — short, true
// statements about how the app actually works (no invented stats), fading
// between phrases every few seconds. Purely decorative copy, not user data.
const TICKER_PHRASES = [
  'No passwords to leak, guess, or reuse.',
  'Sign in with a passkey — Face ID, Touch ID, or your device PIN.',
  'One-time codes expire in minutes, not forever.',
...
function HeroTicker() {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setFade(false);
      setTimeout(() => { setI((p) => (p + 1) % TICKER_PHRASES.length); setFade(true); }, 220);
    }, 3200);
    return () => clearInterval(t);
  }, []);
  return (
    <p className={`text-xs md:text-[13px] text-emerald-300/90 font-mono-code transition-opacity duration-200 ${fade ? 'opacity-100' : 'opacity-0'}`}>
      &gt; {TICKER_PHRASES[i]}
    </p>
  );
}

// Faint drifting accent dots behind the hero — restrained, slow, and never
// more than a handful on screen at once. Adds a "slightly futuristic" sense
// of depth without a 3D centerpiece or particle-system spectacle.
const FLOAT_DOTS = [
  { top: '12%', left: '8%', size: 3, delay: '0s', dur: '9s' },
  { top: '68%', left: '4%', size: 2, delay: '1.2s', dur: '11s' },
  { top: '24%', left: '92%', size: 3, delay: '2.1s', dur: '10s' },
  { top: '78%', left: '90%', size: 2, delay: '0.6s', dur: '8s' },
  { top: '46%', left: '96%', size: 2, delay: '3s', dur: '12s' },
  { top: '90%', left: '50%', size: 2, delay: '1.8s', dur: '9.5s' },
];
function FloatingDots() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
      {FLOAT_DOTS.map((d, idx) => (
        <span
          key={idx}
          className="absolute rounded-full bg-emerald-400/40 animate-float-dot"
          style={{
...
export default function AuthScreen({ onAuthSuccess }) {
  ...
  // Normalizes free-typed phone input to a canonical E.164-ish shape before
  // it ever reaches the backend: strips spaces/dashes, keeps only digits in
  // the number, ensures the country code always starts with "+". This keeps
  // send-otp and verify-otp looking up the exact same stored number.
  const normalizedPhone = () => {
    const suffix = `+${String(phoneSuffix).replace(/[^\d]/g, '')}`;
    let number = String(phoneNumber).replace(/[^\d]/g, '');
    if (number.startsWith('0') && number.length > 10) number = number.replace(/^0+/, '');
  ...
  const handleSendOtp = async (e) => {
    ...
    if (method === 'email') {
      ...
      const { suffix, number } = normalizedPhone();
      if (number.length < 6) {
        setError("That phone number doesn't look valid. Include your country code and full number.");
        setLoading(false);
        return;
      }
      // Keep displayed state in sync with what we're actually sending so the
      // verify-otp call below (and the "sent to +91 XXXXX" label) uses the
      // exact same canonical number.
      setPhoneSuffix(suffix);
      setPhoneNumber(number);
      body = { phoneNumber: number, phoneSuffix: suffix };
    }
  ...
  const handlePasskeySignIn = async () => {
    if (!passkeySupported) {
      setError('This browser or device does not support passkeys (WebAuthn). Try a recent Chrome, Safari, or Edge.');
      return;
    }
    ...
    setInfoMessage('');
    try {
      const optRes = await fetch(`${API_BASE}/auth/passkeys/login-options`, { method: 'POST' });
      const optData = await optRes.json();
      if (optData.status !== 'success') { setError(optData.message || 'Unable to start passkey sign-in.'); return; }
      ...
      if (verifyData.status === 'success' && verifyData.data) {
        onAuthSuccess(verifyData.data.user, verifyData.data.token);
      } else {
        setError(verifyData.message || 'Passkey sign-in failed.');
      }
    } catch (err) {
      console.error(err);
      if (err.name === 'NotAllowedError') setError('Passkey sign-in was cancelled or no passkey is available on this device.');
      else setError('Passkey sign-in failed on this device.');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#080A0D] text-[#F5F7FA] font-sans select-none relative overflow-x-hidden grid-bg">
      ...
      {/* 2. HERO SECTION — marketing column + login card */}
      <div className="min-h-[calc(100vh-80px)] flex flex-col items-center justify-center px-4 py-12 relative z-20 overflow-hidden">
        <FloatingDots />

        <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-[1.05fr_400px] gap-10 lg:gap-14 items-center relative z-10">

          {/* Marketing column — headline, live ticker, trust badges. This is
              what the hero was missing: a reason to care before the form. */}
          <div className="hidden lg:block space-y-6">
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[10px] font-mono-code text-emerald-400 font-semibold uppercase tracking-widest">
              <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse" /> Passwordless by default
            </span>
            <h1 className="text-4xl xl:text-[42px] font-extrabold text-white tracking-tight leading-[1.08]">
              A workspace that never asks for a password.
            </h1>
            <p className="text-sm text-slate-400 leading-relaxed max-w-md">
              Chat, share files, and stay in sync with your team — authenticated by email, phone, or a passkey your device already trusts.
            </p>
            <div className="h-5"><HeroTicker /></div>
            <div className="flex flex-wrap items-center gap-2 pt-2">
              {['WebAuthn Passkeys', 'One-Time Codes', 'End-to-End Encrypted'].map((badge) => (
                <span key={badge} className="px-2.5 py-1 rounded-lg bg-[#101318] border border-[#1f232b] text-[10.5px] font-mono-code text-slate-400">{badge}</span>
              ))}
            </div>
          </div>

        {/* Perspective container for 3D Tilt */}
        <div className="w-full max-w-[400px] perspective-1000 mx-auto lg:mx-0">
          <div
            ref={cardRef}
          ...
          >
              ...
              <form onSubmit={handleVerifyOtp} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <p className="text-[11px] text-slate-500">
                    ...
                    <span className="text-slate-300 font-mono-code">{method === 'email' ? email : `${phoneSuffix} ${phoneNumber}`}</span>
                  </p>
                    ...
                    {Array.from({ length: 6 }).map((_, i) => (
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setOtp(joined);
                          if (digit && i < 5) otpBoxRefs.current[i + 1]?.focus();
                        }}
                        onKeyDown={(e) => {
                        ...
                        className="w-full aspect-square px-0 py-2.5 bg-[#080A0D] border border-[#1f232b] focus:border-emerald-500/60 rounded-xl text-slate-100 text-center font-mono-code text-lg outline-none transition-colors"
                      />
                    ))}
                  </div>
                  <div className="text-right">
                    <button
                      ...
                      className="text-[11px] font-mono-code text-emerald-400 hover:text-emerald-300 disabled:text-slate-600 disabled:cursor-not-allowed transition-colors"
                    >
            ...
            <div className="pt-2">
              ...
              <button
                ...
                className="w-full py-2 px-3 bg-[#080A0D] border border-[#1f232b] hover:border-slate-700 rounded-xl flex items-center justify-between text-xs text-slate-300 transition-colors disabled:opacity-60"
              >
                <div className="flex items-center gap-2">
                  <KeyRound className="w-3.5 h-3.5 text-emerald-400" />
                  <span>{passkeyLoading ? 'Waiting for device…' : 'Sign in with Passkey'}</span>
                </div>
                {!passkeyLoading && <span className="px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[9px] font-mono-code text-emerald-400 font-semibold">Recommended</span>}
              </button>
            </div>
      ...
      <Reveal as="section" id="features" className="w-full max-w-4xl mx-auto px-6 py-20 border-t border-[#1f232b]/50">
        <div className="mb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="hover-lift p-5 rounded-2xl bg-[#101318] border border-[#1f232b] hover:border-emerald-500/30 transition-all space-y-2">
      ...
      <Reveal as="section" id="workspace" className="w-full max-w-4xl mx-auto px-6 py-20 border-t border-[#1f232b]/50">
        <div className="mb-10">
      ...
      <Reveal as="section" className="w-full max-w-4xl mx-auto px-6 py-20 border-t border-[#1f232b]/50">
        <div className="mb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
          <div className="hover-lift p-5 rounded-2xl bg-[#101318] border border-[#1f232b] space-y-3">
      ...
      {/* 6. SECTION 4: FAQ */}
      <Reveal as="section" id="faq" className="w-full max-w-4xl mx-auto px-6 py-20 border-t border-[#1f232b]/50">
        <div className="mb-10">
      ...
      <Reveal as="section" className="w-full max-w-4xl mx-auto px-6 py-12 border-t border-[#1f232b]/50">
