...
export default function FilesView({ user, files, fetchFiles, allUsers }) {
              ...
              return (
                <div key={f._id} className="flex items-center gap-3 px-4 py-3 border-b border-[var(--border-subtle)]/50 last:border-0 hover:bg-[var(--bg-hover)] hover:translate-x-0.5 transition-all">
                  <div onClick={() => setPreviewFile(f)} className="w-10 h-10 rounded-lg bg-[var(--bg-surface-2)] border border-[var(--border-subtle)] flex items-center justify-center text-emerald-400 shrink-0 cursor-pointer overflow-hidden">
                    {f.fileType === 'image' ? <img src={f.fileUrl} alt={f.filename} className="w-full h-full object-cover" /> : <Icon className="w-4 h-4" />}
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <a href={f.fileUrl} download={f.filename} title="Download" className="p-1.5 rounded-lg border border-[var(--border-subtle)] hover:bg-[var(--bg-hover)] text-slate-400 hover:text-slate-200"><Download className="w-3.5 h-3.5" /></a>
                    {isMine && (
      ...
      {previewFile && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-6" onClick={() => setPreviewFile(null)}>
          <div className="relative max-w-2xl w-full" onClick={(e) => e.stopPropagation()}>
            ...
            {previewFile.fileType === 'image' && <img src={previewFile.fileUrl} alt={previewFile.filename} className="max-h-[75vh] w-auto mx-auto rounded-xl" />}
            {previewFile.fileType === 'video' && <video src={previewFile.fileUrl} controls autoPlay className="max-h-[75vh] w-full mx-auto rounded-xl" />}
            {(previewFile.fileType === 'document' || previewFile.fileType === 'other') && (
              <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl p-8 text-center">
                ...
                <a href={previewFile.fileUrl} download={previewFile.filename} className="inline-flex items-center gap-1.5 mt-4 text-xs font-semibold px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950"><Download className="w-3.5 h-3.5" /> Download</a>
              </div>
            )}
          </div>
        </div>
            ...
            <div className="max-h-64 overflow-y-auto custom-scrollbar p-2">
              {allUsers.length === 0 ? (
                ...
                <div key={u._id} onClick={() => handleShare(shareTarget._id, [u._id])} className="flex items-center gap-2.5 p-2 rounded-lg cursor-pointer hover:bg-[var(--bg-hover)]">
                  <img src={u.profilePicture || `https://api.dicebear.com/7.x/adventurer/svg?seed=${u.username}`} alt={u.username} className="w-7 h-7 rounded-lg object-cover" />
