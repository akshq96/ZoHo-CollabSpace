...
const { isBlockedEitherWay } = require('./connectionController');
...
const sendMessage = async (req, res) => {
    ...
    try {
        ...
        // A block is the one thing that stops messaging retroactively, even
        // on a conversation that already existed. Frontend hides the
        // composer for a blocked contact, but this is the real enforcement.
        if (!conversation.isGroup && activeReceiverId) {
            if (await isBlockedEitherWay(senderId, activeReceiverId)) {
                return response(res, 403, "You can't message this user.");
            }
        }
        ...
        // Handle attachment file if present. The File record is created
        // FIRST, from the exact bytes multer just received for THIS request
        // (req.file.buffer never touches any other request), and the message
        // references that record's id directly. This is what guarantees the
        // image/video/file rendered in chat is always the exact one the
        // sender picked — there is no shared/reused URL or filename lookup
        // anywhere in this path that could resolve to a different upload.
        if (req.file) {
            ...
            if (cloudinaryConfigured()) {
                ...
                const saved = saveBuffer(req.file.buffer, req.file.originalname, req.file.mimetype, 'chat');
                mediaUrl = saved.url;
            }

            // Track chat attachments in the shared Files library too, so the
            // Files page (All / Images / Videos / Documents / Shared with me)
            // reflects real files exchanged in conversations. The Message
            // below links directly to this exact record via `attachment`.
            try {
...
const searchMessages = async (req, res) => {
    ...
    if (!q || !q.trim()) {
        return response(res, 200, "No query provided", []);
    }

    try {
        // Only search conversations this user is actually a participant of.
        const myConversations = await Conversation.find({ participants: userId }).select('_id isGroup groupName participants');
        ...
        return response(res, 200, "Messages searched successfully", messages);
    } catch (error) {
...
module.exports = { sendMessage, getMessages, markMessagesAsSeen, searchMessages };
